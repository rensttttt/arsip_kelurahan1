...
db_config = {
    ...
    'database': app.config.get('DB_NAME', 'database_name'),
...
def dashboard():
    ...
    try:
        ...
        flash(f'Terjadi kesalahan saat memuat dashboard (ID: {error_id}). Silakan coba lagi.', 'danger')
...
def api_dashboard():
    ...
    try:
        ...
        logging.error(f"[{error_id}] API Dashboard unexpected error: {e}", exc_info=True)
        log_system_error(
            module="API Dashboard",
...
def activity(action_filter=None):
    ...
    valid_actions = [
        ...
        'profile_update', 'password_change', 'view_activity'
    ]
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            logging.error(f"[{error_id}] Activity route: Database connection failed")
            log_system_error(
                module="Activity",
        ...
        logging.error(f"[{error_id}] Activity route MySQL error: {e}", exc_info=True)
        log_system_error(
            module="Activity",
            message=f"MySQL error [{error_id}]: {str(e)}",
            ip_address=request.remote_addr or 'unknown',
        ...
        if e.errno == mysql.connector.errorcode.ER_NO_SUCH_TABLE:
            flash(f'Tabel database tidak ditemukan (ID: {error_id}). Hubungi administrator.', 'danger')
        elif e.errno == mysql.connector.errorcode.ER_ACCESS_DENIED_ERROR:
            flash(f'Izin akses database ditolak (ID: {error_id}). Hubungi administrator.', 'danger')
        else:
        ...
        logging.error(f"[{error_id}] Activity route unexpected error: {e}", exc_info=True)
        log_system_error(
            module="Activity",
            message=f"Unexpected error [{error_id}]: {str(e)}",
            ip_address=request.remote_addr or 'unknown',
...
def profile():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            if conn:
                conn.rollback()
            logger.error(f"Profile update MySQL error: {e}", exc_info=True)
            log_system_error("Profile Update", f"MySQL error: {e}")
            flash('Terjadi kesalahan pada database. Silakan coba lagi.', 'danger')

        except Exception as e:
            logger.error(f"Profile update unexpected error: {e}", exc_info=True)
...
def change_password():
    ...
    if not is_valid_password(new_password):
        flash('Password must be at least 8 characters with uppercase, lowercase, numbers, and special characters.', 'danger')
        return redirect(url_for('profile'))  # jangan lupa return supaya tidak lanjut ke proses berikutnya
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            log_system_error(
                ...
                ip_address=request.remote_addr or 'unknown',
                user_id=current_user.id,
                details={'route': request.path, 'sort': sort, 'query': query, 'category': category}
            )
        ...
        if page > total_pages:
            ...
            return redirect(url_for('archive_list', page=total_pages, sort=sort, query=query, category=category, date_range=date_range))
        ...
        if e.errno == mysql.connector.errorcode.ER_NO_SUCH_TABLE:
            flash(f'Tabel arsip tidak ditemukan (ID: {error_id}). Hubungi administrator.', 'danger')
        elif e.errno == mysql.connector.errorcode.ER_ACCESS_DENIED_ERROR:
            flash(f'Izin akses database ditolak (ID: {error_id}). Hubungi administrator.', 'danger')
        elif e.errno == mysql.connector.errorcode.ER_BAD_FIELD_ERROR:
            flash(f'Kolom database tidak valid (ID: {error_id}). Hubungi administrator.', 'danger')
        else:
            flash(f'Terjadi kesalahan database (ID: {error_id}). Silakan coba lagi.', 'danger')
...
def format_file_size(size):
    ...
    try:
        ...
        logger.warning(f"Invalid file_size: {size}")
        return 'Unknown'
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            try:
                ...
                logger.debug(f"[{error_id}] Metadata berhasil disimpan dengan archive_id: {archive_id}")
...
def archive_detail(archive_id):
    ...
    try:
        ...
        archive['updated_at'] = archive['updated_at'].strftime('%d %B %Y, %H:%M') if archive['updated_at'] else '-'
...
def download_archive(archive_id):
    ...
    try:
        ...
        # Validate file path
        file_path = os.path.join(current_app.config.get('UPLOAD_FOLDER', 'uploads'), archive['file_path'])
        ...
        cursor.execute(
            ...
            VALUES (%s, %s, %s, %s, NOW())
            """,
            (archive_id, current_user.id, 'download', request.remote_addr or 'unknown')
        )
        conn.commit()
        ...
        logger.error(f"[{error_id}] Archive download MySQL error: {e}", exc_info=True)
        log_system_error(
            module="Archive",
            message=f"MySQL error [{error_id}]: {str(e)}",
            ip_address=request.remote_addr or 'unknown',
        ...
        logger.error(f"[{error_id}] Archive download unexpected error: {e}", exc_info=True)
        log_system_error(
            module="Archive",
            message=f"Unexpected error [{error_id}]: {str(e)}",
            ip_address=request.remote_addr or 'unknown',
...
def admin_user_management():
    ...
    if request.method == 'POST':
        try:
            ...
            if not form.csrf_token.data:
                logger.warning(f"[{error_id}] Missing CSRF token in form submission")
                flash('CSRF token tidak ditemukan. Silakan coba lagi.', 'danger')
                return redirect(url_for('admin_user_management'))

        except Exception as e:
            logger.warning(f"[{error_id}] CSRF validation failed: {e}")
        ...
        try:
            ...
            if action == 'delete':
                ...
                if not user_id or not user_id.isdigit():
                    logger.warning(f"[{error_id}] Invalid user_id in delete request: {user_id}")
                ...
                cursor.execute("SELECT id, username, is_superadmin FROM user WHERE id = %s", (user_id,))
                ...
                try:
                    ...
                    log_activity(
                        ...
                        details={'deleted_user_id': user_id, 'deleted_username': user_to_delete['username']}
                    )
                    flash('Pengguna berhasil dihapus!', 'success')
                    return redirect(url_for('admin_user_management'))
                except Exception as e:
                    conn.rollback()
                    logger.error(f"[{error_id}] Delete user failed: {e}")
            ...
            if form.validate_on_submit():
                ...
                try:
                    if user_id:
                        ...
                        cursor.execute("""
                            ...
                            SET username = %s, email = %s, full_name = %s, phone = %s, 
                                ...
                                updated_at = %s
                            WHERE id = %s
                        """, (username, email, full_name, phone, is_admin, is_superadmin, is_active, current_time, user_id))
                        ...
                        log_activity(
                            ...
                            details={'username': username, 'user_id': int(user_id), 
                                     'changes': {'is_admin': is_admin, 'is_superadmin': is_superadmin, 'is_active': is_active}}
                        )
                        ...
                        """, (username, email, password_hash, full_name, phone, 
                              is_admin, is_superadmin, is_active, current_time))
                        ...
                        flash(f'Pengguna berhasil dibuat! Kata sandi sementara: {temp_password}', 'success')
                    ...
                    conn.rollback()
                    logger.error(f"[{error_id}] Create/Update user failed: {e}")
            ...
            if conn and conn.in_transaction:
                conn.rollback()
            logger.error(f"[{error_id}] Admin user management error: {e}")
            flash(f'Terjadi kesalahan saat memproses permintaan: {str(e)}', 'danger')
            return redirect(url_for('admin_user_management'))
    ...
    try:
        ...
        logger.error(f"[{error_id}] Admin user management load error: {e}")
...
def admin_archive_management(archive_id=None):
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            flash('Gagal terhubung ke database.', 'danger')
            return render_template('admin_archive_management.html', archives=[], title='Archive Management')
        ...
        if request.method == 'POST':
            ...
            if action == 'delete' and archive_id:
                ...
                cursor.execute("SELECT title, user_id FROM archives WHERE id = %s", (archive_id,))
                ...
                if archive_info:
                    ...
                    log_activity(
                        ...
                        details=json.dumps({'archive_id': int(archive_id), 'archive_title': archive_info['title']})
                    )
        ...
        if archive_id is not None:
            ...
            try:
                ...
                logger.error(f"[{error_id}] Failed to send file: {e}", exc_info=True)
        ...
        logger.error(f"[{error_id}] Admin archive management error: {e}", exc_info=True)
        log_system_error(
...
def admin_system_settings():
    ...
    try:
        ...
        if not conn or not conn.is_connected():
            ...
            flash('Gagal terhubung ke database.', 'danger')
            return render_template('admin_system_settings.html', settings=[], title='System Settings')
            ...
            if setting_key and setting_value:
                ...
                if existing:
                    ...
                    cursor.execute("""
                        ...
                        WHERE key = %s
                    """, (setting_value, setting_description, current_time, setting_key))
                    action_msg = 'updated'
                else:
                    # Create new setting
                    cursor.execute("""
                ...
                log_activity(
                    ...
                    details=json.dumps({'key': setting_key, 'value': setting_value, 'action': action_msg})
                )
        ...
        logger.error(f"[{error_id}] Admin system settings error: {e}", exc_info=True)
        log_system_error(
...
def api_admin_recent_notifications():
    ...
    try:
        ...
        logger.error(f"API recent notifications error: {e}")
        return jsonify({'error': 'Internal server error'}), 500
